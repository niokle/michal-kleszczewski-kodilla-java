    ...
    public void processOrder(final OrderDto orderDto, final Long userId) throws OrderProcessingException {
        ...
        try {
            for (ItemDto itemDto : orderDto.getItems()) {
                LOGGER.info("Additing item " + itemDto.getProductId() + ", " + itemDto.getQuantity() + " pcs");
            ...
            if (!shopService.submitOrder(orderId)) {
                LOGGER.error(OrderProcessingException.ERR_SUBMINTING_ERROR);
                wasError = true;
                throw new OrderProcessingException(OrderProcessingException.ERR_SUBMINTING_ERROR);
            }
