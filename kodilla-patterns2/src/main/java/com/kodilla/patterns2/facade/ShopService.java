...
public class ShopService {
    ...
    @Autowired
    public Authanticator authanticator;

    @Autowired
    public ProductServiceClass productServiceClass;

    public Long openOrder(Long userId) {
        if (authanticator.isAuthenticated(userId)) {
            Long maxOrder = (long) orders.stream()
                    .mapToInt(o -> o.getOrderId().intValue())
                    .max().orElse(0);
            orders.add(new Order(maxOrder + 1, userId, productServiceClass));
            return maxOrder + 1;
        } else {
            return -1L;
        }
